...
#  include "zdfddm_substitute.h90"
   ...
   !! $Id: step.F90 3217 2011-12-13 09:07:25Z cbricaud $
...
CONTAINS
   ...
   SUBROUTINE stp( kstp )
      ...
      LOGICAL ::   ll_exist ! for hacking nitend
      LOGICAL, SAVE :: ll_diawr2=.TRUE. ! for diawri2 call
      LOGICAL, SAVE :: ll_diawr3=.TRUE. ! for diawri2 call
      !! ---------------------------------------------------------------------
      IF ( nn_write2   == 0 .AND. kstp == nit000 ) ll_diawr2=.FALSE.
      IF ( nn_write3   == 0 .AND. kstp == nit000 ) ll_diawr3=.FALSE.
#if defined key_agrif
...
# endif   
      ...
      IF( ll_diawr2  )   CALL dia_wri_2( kstp )       ! ocean model: outputs [with nn_write2 frequency]
      IF( ll_diawr3  )   CALL dia_wri_3( kstp )       ! ocean model: outputs [with nn_write3 frequency]
...
!}
      ...
      IF ( MOD(kstp,nn_write) == 0 ) THEN   ! check every nn_write only
!        Hack to change nitend on the fly. It works but must be managed with caution.
!         * nitend must correspond to the end of writing sequence.
!         * script must manage this file : initialization, take it into account for db
         INQUIRE(FILE='nitend.txt', EXIST=ll_exist )
         IF ( ll_exist ) THEN
            OPEN(9999,FILE='nitend.txt') ; READ(9999,*) nitend
            nitend = nit000 - 1 + ( MAX( INT((nitend -nit000)/REAL(nn_write)+1) , 1) ) * nn_write
            nitrst = nitend ; CLOSE (9999)
         ENDIF
      ENDIF
