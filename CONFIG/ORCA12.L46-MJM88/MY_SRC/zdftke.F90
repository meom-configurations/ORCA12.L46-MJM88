MODULE zdftke
   ...
   !{ DRAKKAR bug fix from Romain Bourdalle Badie
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: avt_tke
   REAL(wp)        , ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: avm_tke
   REAL(wp)        , ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: avmu_tke
   REAL(wp)        , ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: avmv_tke
   !}
#if defined key_c1d
   !                                                                        !!** 1D cfg only  **   ('key_c1d')
...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: zdftke.F90 3231 2011-12-21 09:11:11Z smasson $
...
CONTAINS
   INTEGER FUNCTION zdf_tke_alloc()
      ...
      ALLOCATE(                                                                    &
         ...
         &   avm_tke (jpi,jpj,jpk) , avt_tke   (jpi,jpj,jpk) ,                     &
         &   avmu_tke(jpi,jpj,jpk) , avmv_tke  (jpi,jpj,jpk) ,                     &
...
!}
      ...
      DO jk = 2, jpkm1           !* Shear production at uw- and vw-points (energy conserving form)
         DO jj = 1, jpj                 ! here avmu, avmv used as workspace
            DO ji = 1, jpi
               avmu_tke(ji,jj,jk) = avmu_tke(ji,jj,jk) * ( un(ji,jj,jk-1) - un(ji,jj,jk) )   &
                  &                            * ( ub(ji,jj,jk-1) - ub(ji,jj,jk) )   & 
                  &           / (  fse3uw_n(ji,jj,jk)         &
                  &              * fse3uw_b(ji,jj,jk) )
               avmv_tke(ji,jj,jk) = avmv_tke(ji,jj,jk) * ( vn(ji,jj,jk-1) - vn(ji,jj,jk) )   &
      ...
      DO jk = 2, jpkm1           !* Matrix and right hand side in en
         DO jj = 2, jpjm1
            DO ji = fs_2, fs_jpim1   ! vector opt.
               ...
               zzd_up = zcof * ( avm_tke  (ji,jj,jk+1) + avm_tke  (ji,jj,jk  ) )   &  ! upper diagonal
                  &          / ( fse3t(ji,jj,jk  ) * fse3w(ji,jj,jk  ) )
               zzd_lw = zcof * ( avm_tke  (ji,jj,jk  ) + avm_tke  (ji,jj,jk-1) )   &  ! lower diagonal
                  &          / ( fse3t(ji,jj,jk-1) * fse3w(ji,jj,jk  ) )
                  !                                                           ! shear prod. at w-point weightened by mask
               zesh2  =  ( avmu_tke(ji-1,jj,jk) + avmu_tke(ji,jj,jk) ) / MAX( 1._wp , umask(ji-1,jj,jk) + umask(ji,jj,jk) )   &
                  &    + ( avmv_tke(ji,jj-1,jk) + avmv_tke(ji,jj,jk) ) / MAX( 1._wp , vmask(ji,jj-1,jk) + vmask(ji,jj,jk) )    
                  !
               ...
               en(ji,jj,jk) = en(ji,jj,jk) + rdt * (  zesh2  -   avt_tke(ji,jj,jk) * rn2(ji,jj,jk)    &
...
!CDIR NOVERRCHK
            DO ji = fs_2, fs_jpim1   ! vector opt.
               ...
               avm_tke  (ji,jj,jk) = MAX( zav,                  avmb(jk) ) * tmask(ji,jj,jk)
               !{ DRAKKAR 
               avt_tke  (ji,jj,jk) = MAX( zav, ziceavtb(ji,jj) * avtb(jk) ) * tmask(ji,jj,jk)
               !}
               dissl(ji,jj,jk) = zsqen / zmxld(ji,jj,jk)
            END DO
         END DO
      END DO
      CALL lbc_lnk( avm_tke, 'W', 1. )      ! Lateral boundary conditions (sign unchanged)
      !
      DO jk = 2, jpkm1            !* vertical eddy viscosity at u- and v-points
         DO jj = 2, jpjm1
            DO ji = fs_2, fs_jpim1   ! vector opt.
               avmu_tke(ji,jj,jk) = 0.5 * ( avm_tke(ji,jj,jk) + avm_tke(ji+1,jj  ,jk) ) * umask(ji,jj,jk)
               avmv_tke(ji,jj,jk) = 0.5 * ( avm_tke(ji,jj,jk) + avm_tke(ji  ,jj+1,jk) ) * vmask(ji,jj,jk)
            END DO
         END DO
      END DO
      CALL lbc_lnk( avmu_tke, 'U', 1. )   ;   CALL lbc_lnk( avmv_tke, 'V', 1. )      ! Lateral boundary conditions
      !
      IF( nn_pdl == 1 ) THEN      !* Prandtl number case: update avt
         DO jk = 2, jpkm1
            DO jj = 2, jpjm1
               DO ji = fs_2, fs_jpim1   ! vector opt.
                  zcoef = avm_tke(ji,jj,jk) * 2._wp * fse3w(ji,jj,jk) * fse3w(ji,jj,jk)
                  !                                          ! shear
                  zdku = avmu_tke(ji-1,jj,jk) * ( un(ji-1,jj,jk-1) - un(ji-1,jj,jk) ) * ( ub(ji-1,jj,jk-1) - ub(ji-1,jj,jk) )   &
                    &  + avmu_tke(ji  ,jj,jk) * ( un(ji  ,jj,jk-1) - un(ji  ,jj,jk) ) * ( ub(ji  ,jj,jk-1) - ub(ji  ,jj,jk) )
                  zdkv = avmv_tke(ji,jj-1,jk) * ( vn(ji,jj-1,jk-1) - vn(ji,jj-1,jk) ) * ( vb(ji,jj-1,jk-1) - vb(ji,jj-1,jk) )   &
                    &  + avmv_tke(ji,jj  ,jk) * ( vn(ji,jj  ,jk-1) - vn(ji,jj  ,jk) ) * ( vb(ji,jj  ,jk-1) - vb(ji,jj  ,jk) )
...
!!gm              zpdlr = MAX(  0.1_wp,  ri_crit / MAX( ri_crit , zri )  )
                  !{ DRAKKAR 
                  avt_tke(ji,jj,jk)   = MAX( zpdlr * avt_tke(ji,jj,jk), ziceavtb(ji,jj) * avtb(jk) ) * tmask(ji,jj,jk)
                  !}
# if defined key_c1d
...
# endif
         ...
         END DO
      ENDIF
      CALL lbc_lnk( avt_tke, 'W', 1. )                      ! Lateral boundary conditions on avt  (sign unchanged)

      IF(ln_ctl) THEN
         CALL prt_ctl( tab3d_1=en  , clinfo1=' tke  - e: ', tab3d_2=avt_tke, clinfo2=' t: ', ovlap=1, kdim=jpk)
         CALL prt_ctl( tab3d_1=avmu_tke, clinfo1=' tke  - u: ', mask1=umask,                   &
            &          tab3d_2=avmv_tke, clinfo2=       ' v: ', mask2=vmask, ovlap=1, kdim=jpk )
      ENDIF
      !{ DRAKKAR + RBB : avmu, avmv used later on and eventually increased in zdfevd
      avt (:,:,:) = avt_tke(:,:,:)
      avm (:,:,:) = avm_tke(:,:,:)
      avmu(:,:,:) = avmu_tke(:,:,:)
      avmv(:,:,:) = avmv_tke(:,:,:)
   ...
   SUBROUTINE zdf_tke_init
      ...
      DO jk = 1, jpk
         avt_tke (:,:,jk) = avtb(jk) * tmask(:,:,jk)
         avm_tke (:,:,jk) = avmb(jk) * tmask(:,:,jk)
         avmu_tke(:,:,jk) = avmb(jk) * umask(:,:,jk)
         avmv_tke(:,:,jk) = avmb(jk) * vmask(:,:,jk)
      END DO
      dissl(:,:,:) = 1.e-12_wp
      !                              
   ...
   SUBROUTINE tke_rst( kt, cdrw )
        ...
        IF( ln_rstart ) THEN                   !* Read the restart file
           ...
           IF( id1 > 0 ) THEN                       ! 'en' exists
              ...
              IF( MIN( id2, id3, id4, id5, id6 ) > 0 ) THEN        ! all required arrays exist
                 CALL iom_get( numror, jpdom_autoglo, 'avt'  , avt_tke   )
                 CALL iom_get( numror, jpdom_autoglo, 'avm'  , avm_tke   )
                 CALL iom_get( numror, jpdom_autoglo, 'avmu' , avmu_tke  )
                 CALL iom_get( numror, jpdom_autoglo, 'avmv' , avmv_tke  )
     ...
     ELSEIF( TRIM(cdrw) == 'WRITE' ) THEN   ! Create restart file
        ...
        CALL iom_rstput( kt, nitrst, numrow, 'avt'  , avt_tke   )
        CALL iom_rstput( kt, nitrst, numrow, 'avm'  , avm_tke   )
        CALL iom_rstput( kt, nitrst, numrow, 'avmu' , avmu_tke  )
        CALL iom_rstput( kt, nitrst, numrow, 'avmv' , avmv_tke  )
   ...
   REAL, PUBLIC, ALLOCATABLE,  DIMENSION(:,:,:) :: avt_tke
CONTAINS
