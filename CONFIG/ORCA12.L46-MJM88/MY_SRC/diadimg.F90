...
#  include "domzgr_substitute.h90"
   ...
   !! $Id: diadimg.F90 3214 2011-12-12 13:33:31Z cbricaud $
...
CONTAINS
  ...
  SUBROUTINE dia_wri_dimg(cd_name, cd_text, ptab, klev, cd_type , cd_dirout, ksubi, kwrite )
    !!-------------------------------------------------------------------------
    ...
    !!---------------------------------------------------------------------------
    CHARACTER(len=*),                   INTENT(in) :: cd_name  ! dimg file name
    CHARACTER(len=*),                   INTENT(in) :: cd_text  ! comment to write on record #1
    INTEGER,                            INTENT(in) :: klev     ! number of level in ptab to write
    REAL(wp), DIMENSION(:,:,:),         INTENT(in) :: ptab     ! 3D array to write 
    CHARACTER(LEN=1),                   INTENT(in) :: cd_type  ! either 'T', 'W' or '2' 
    !                                                          !  depending on the vertical
    !                                                          ! grid for ptab. 2 stands for 2D file
    CHARACTER(LEN=*),                   INTENT(in) :: cd_dirout
    INTEGER, OPTIONAL, DIMENSION(klev), INTENT(in) :: ksubi 
    INTEGER, OPTIONAL,                  INTENT(in) :: kwrite   ! nn_write or nn_write2

    !! * Local declarations
    INTEGER  :: jk, jn          ! dummy loop indices
    INTEGER  :: irecl4          ! record length in bytes
    INTEGER  :: inum            ! logical unit (set to 14)
    INTEGER  :: irec            ! current record to be written
    INTEGER  :: irecend         ! record number where nclit... are stored
    INTEGER  :: iwri            ! record number where nclit... are stored

    REAL(sp) :: zdx, zdy        ! single precision variable for header
    REAL(sp) :: zspval, zwest   ! "                  "
...
!!!! jm34?   inum = 14
    ...
    zsouth=njmpp * 1._sp
    zwest=nimpp * 1._sp
    zdtwr=iwri * rn_rdt  / 86400_wp   ! write step in days
    ...
    IF ( ln_dimgnnn  ) THEN
     ...
     irecl4 = MAX(jpi*jpj*sp , 84+(18+jpk)*sp + 8*jpnij*sp +3 *sp  )
       ...
       CALL ctl_opn(inum, clname,'UNKNOWN','UNFORMATTED','DIRECT',irecl4,numout,lwp, cdirout=cd_dirout)
    ...
    ELSE
       ...
       !! * Open file
       CALL ctl_opn(inum, cd_name,'UNKNOWN','UNFORMATTED','DIRECT',irecl4,numout,lwp, cdirout=cd_dirout)
