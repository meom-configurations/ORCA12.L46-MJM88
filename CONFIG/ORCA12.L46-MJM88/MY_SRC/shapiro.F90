MODULE shapiro 
  ...
  SUBROUTINE Shapiro_1D(ptabin, kiter, cd_overlap, ptabout) !GIG
      ...
      REAL(wp)                              :: zalpha     ! weight coeficient (y direction)
            ...
            DO jj = 2,jpjm1
               DO ji = 2,jpim1
                  ! We crop on the coast
                   znum = zvarout(ji,jj)   + zalpha * ( &
                            (zvarout(ji-1,jj  )-zvarout(ji,jj))*tmask(ji-1,jj  ,1)  &
                          + (zvarout(ji+1,jj  )-zvarout(ji,jj))*tmask(ji+1,jj  ,1)  &
                          + (zvarout(ji  ,jj-1)-zvarout(ji,jj))*tmask(ji  ,jj-1,1)  &
                          + (zvarout(ji  ,jj+1)-zvarout(ji,jj))*tmask(ji  ,jj+1,1) )
